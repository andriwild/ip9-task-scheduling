    ...
    void drawBackground(QPainter *painter, const QRectF &rect) override {
        ...
        auto getColor = [](int type) -> QColor {
            switch(static_cast<RobotStateType>(type)) {
                case RobotStateType::IDLE: return Qt::lightGray;
                case RobotStateType::MOVING: return QColor(100, 200, 100);
        ...
        for (const auto& block : m_states) {
            if (timeToX(block.endTime) < rect.left()) continue;
            if (timeToX(block.startTime) > rect.right()) continue;
        ...
        if (firstTick < loopStart) firstTick += tickStep;
...
private:
    void setupButtons() {
        ...
        m_btnZoomIn->setStyleSheet("QPushButton { font-weight: bold; font-size: 16px; background-color: rgba(255, 255, 255, 200); border: 1px solid #999; border-radius: 4px; } QPushButton:hover { background-color: white; }");
        ...
        m_btnZoomOut->setStyleSheet("QPushButton { font-weight: bold; font-size: 16px; background-color: rgba(255, 255, 255, 200); border: 1px solid #999; border-radius: 4px; } QPushButton:hover { background-color: white; }");
    ...
    void updateScene() {
        ...
        m_scene->setSceneRect(-TIMELINE_SCENE_MARGIN, 0, totalWidth + (TIMELINE_SCENE_MARGIN * 2), TIMELINE_HEIGHT);
    ...
    void drawMeetingPlan(const des::Appointment& appt, int startTime) {
        ...
        auto rect = m_scene->addRect(startX, Y_LINE_POS, durationWidth, -MARKER_HEIGHT, 
