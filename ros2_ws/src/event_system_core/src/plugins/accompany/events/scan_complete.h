...
public:
    ...
    void execute(ISimContext& ctx) override {
        ...
        DES_LOG_DEBUG(rclcpp::get_logger("des.plugin.accompany.search"), "ScanComplete t=%d room=%s person=%s personInRoom=%d remaining=%d -> tickBT", this->time, ctx.getRobot()->getLocation().c_str(), personName.c_str(), personPresent, m_remainigSearchTime);
        if (personPresent) {
            if (!ctx.getRobot()->isPersonVisible()) {
                DES_LOG_DEBUG(rclcpp::get_logger("des.plugin.accompany.search"), "Found %s in %s", personName.c_str(), ctx.getRobot()->getLocation().c_str());
            }
