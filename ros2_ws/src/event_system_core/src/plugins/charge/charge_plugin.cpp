...
void ChargePlugin::onMissionStart(ISimContext& ctx, IOrder& /*order*/) {
    ctx.changeRobotState(std::make_unique<ChargeState>());
}

void ChargePlugin::onMissionEnd(ISimContext& ctx, IOrder& /*order*/) {
    ctx.changeRobotState(std::make_unique<IdleState>());
}

void ChargePlugin::onStartDriveEvent(ISimContext& /*ctx*/, IOrder& /*order*/) {}

void ChargePlugin::onStopDriveEvent(ISimContext& /*ctx*/, IOrder& /*order*/) {}

OrderPtr ChargePlugin::fromJson(const nlohmann::json& /*j*/) const {
...
int ChargePlugin::planDispatchTime(const IOrder& order, const Scheduler& /*scheduler*/, const std::string& /*startPos*/) const {
    return order.dispatchTime;
}

bool ChargePlugin::isFeasible(const IOrder& /*order*/, const ISimContext& /*context*/) const {
    return true;
}

std::optional<std::string> ChargePlugin::targetLocation(const IOrder& order) const {
